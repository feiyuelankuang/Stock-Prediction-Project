...
class LSTM:
    def __init__(self, ts_dir, vec_dir, kg_dir, parameters,stock_index=None):
        ...
        self.valid_index = math.ceil((self.ts_data.shape[1] - self.seq) * 0.6) # 60-20-20 Split
    ...
    def train(self):
        ...
        with tf.device(device_name):
            ...
            ts_feature = tf.placeholder(tf.float32, [self.batch_size, self.seq, self.ts_data.shape[2]])
            #vec_feature = tf.placeholder(tf.float32, [self.batch_size, self.seq, self.vec_data.shape[2], self.vec_data.shape[3]])
            kg_feature = tf.placeholder(tf.float32, [self.batch_size, self.seq, self.kg_data.shape[2]])
        ...
        for epoch in range(self.parameters['epochs']):
            ...
            ### TRAINING SET ###
            batch_index = np.arange(start=self.seq, stop=self.valid_index, dtype=int)
            np.random.shuffle(batch_index) # Randomly shuffle training set each epoch

            train_loss = 0.0

            for i in range(self.seq, self.valid_index):
                #ts_batch, vec_batch, kg_batch, gt_batch = self.get_batch(batch_index[i - self.seq])
                ts_batch, kg_batch, gt_batch = self.get_batch(batch_index[i - self.seq])
